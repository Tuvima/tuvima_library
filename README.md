...
- **Story-first organization:** Home highlights what to resume, then Watch, Read, Listen, placed Collections & Lists, and new arrivals. Home alone uses the premium cinematic hero. Read, Watch, and Listen each keep one persistent lane title and Discover/media navigation while the content underneath changes. Their Discover surfaces are compact, rail-navigated, and row-based; direct routes such as `/read/books`, `/read/comics`, `/watch/movies`, `/watch/tv`, `/watch/series`, `/listen/music`, and `/listen/audiobooks` open the complete filterable library beneath that same header in one click and use vertically wrapping tiled results rather than horizontally scrolling media rows. Direct tiled results keep their cover or group artwork unchanged and use only a purple hover/focus glow; cinematic background expansion remains on Home and Discover shelves. Listen is album-first: `/listen/music` opens the filterable album grid, while individual tracks remain available through Albums/Songs/Artists shortcuts and album details. Ingestion creates lane shelves from trusted QIDs, provider IDs, or local grouping metadata; Collections appear only when a broader world spans multiple shelves. Series and collection cards use one fixed-size landscape treatment with two to four slightly angled representative covers.
