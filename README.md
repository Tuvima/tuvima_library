...
- **One dashboard:** Home, media lanes, Search, detail pages, Settings/Admin, and Review Queue work together instead of forcing every correction into a separate management workspace. TV show and episode detail pages share the same season selector and list only episodes present in the library. The show hero follows the episode that Watch/Resume will open, identifies it as `Sx Ey`, and uses its synopsis; the broader show description remains in Series information below. Movie heroes use the movie description directly. Continue cards retain the episode still and playback target and identify the child as `Sx Ey`.
