...
- **Unified details:** Media, people, series, and standard collections open on canonical full-width `/details/...` routes. On desktop the cinematic stage fills the visible viewport. Its top-anchored landscape art extends beneath translucent global and lower navigation, with a strong bottom fade and left scrim protecting the copy. Album tracks and audiobook chapters share that stage with identity and a bounded, borderless scrolling list; redundant Tracks/Chapters tabs, album Editions, Play All, and the list's duplicate My List action are omitted while audiobook chapter and player tools remain available. Structural details render their sequence, owned-works, or collection array below the stage on Overview only. Collections show up to four stacked representative artworks in the shared hero, comics match book cover size, and person portraits use the larger visual envelope. Overview combines the full description or biography, a purpose-built cast/credits peek, and applicable series or collection context; recommendations live in a separate Related tab. Jump to appears beside the array heading only when a sequence contains more than ten entries, while alternate series/arc selectors stay left and Show missing stays right. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane. A person's works use managed portrait cover/poster art or square album art—never cinematic landscape backgrounds—and include only owned canonical eligible credits with all eligible roles per work. Standard collection details resolve membership through the same catalog rules as the Collections surface. Listen playlists remain specialized lane-local queue and editing surfaces.
- **Cinematic continuity:** Home and detail heroes share the same full-viewport identity/content composition—logo scale, facts, action, progress, and full-paragraph synopsis—plus the same top-anchored backdrop, left-edge scrim, atmosphere, and top/bottom contrast layers. Home has no lane submenu; its shelves rise into the bottom fade. The non-control slide background opens details, while Watch, Read, Listen, Play, Resume, and Restart act directly. Carousel context sits at the top right as Featured Content or lane-specific Continue wording. Watch landing TV slides always use show-level managed artwork rather than episode stills, and detail tabs use a translucent lower navigation bar.
