...
- **Unified details:** Media, people, series, and standard collections open on canonical full-width `/details/...` routes. On desktop the shared detail stage targets 70% of the visible viewport, capped at 760 px. Album tracks and audiobook chapters share that bounded stage with the identity hero, scroll internally, and never overlap the navigation at its bottom. Structural arrays—episodes, series sequences, collection items, appearances, and a person's owned works—begin intact below the stage rather than cutting the hero in half. Portrait covers stay deliberately smaller inside the larger cinematic field, and existing named Jump to selectors remain part of long sequences. Overview then combines the full description or biography, a cast/credits peek, series or collection context, and related content. Person works include only owned library media from canonical eligible credits and retain all of the person's roles per work. Listen playlists remain specialized lane-local queue and editing surfaces.
