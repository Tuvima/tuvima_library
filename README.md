...
- **Unified details:** Media, people, series, and standard collections open on canonical full-width `/details/...` routes. On desktop the cinematic stage is `95svh`, keeping its lower navigation visible while preserving substantially more of native 16:9 art. Its edge-to-edge, top-center landscape art extends beneath translucent global and lower navigation, with a strong bottom fade and left scrim protecting the copy. Music albums use an album-only version of that stage: a cover-derived blurred atmosphere, a larger sharp cover high on the right, Play and Shuffle on the primary row, utilities beneath them, and a bounded local Tracks/Credits/Details workspace. Its narrower, larger-type track list has no enclosing card or opaque background, retains Show missing, and omits track search so the list feels embedded in the hero. It replaces the shared lower navigation and lower detail body; its optional **More by** block contains at most three other owned albums from the exact canonical primary artist. Audiobooks keep their shared detail structure and specialized chapter/player tools while using a larger cover. Album tracks and audiobook chapters remain embedded rather than shared detail tabs; Editions, Play All, and the list's duplicate My List action are omitted. Structural details render their sequence, owned-works, or collection array below the stage on Overview only, ending in a compact owned summary and an authoritative-total progress bar when available. Collections use no member-derived backdrop, show the same up-to-four representative artwork cluster as collection tiles at a dominant hero scale, expose no collection action buttons, and report the year range, total items, and applicable Read/Watch/Listen counts in one compact row. Comics match book cover size, and person portraits use the larger visual envelope on one continuous no-tab page. Overview combines the full description or biography, a purpose-built cast/credits peek, and applicable series or collection context; recommendations live in a separate Related tab. Jump to appears beside the array heading only when a sequence contains more than ten entries, while alternate series/arc selectors stay left and Show missing stays right. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane. A person's works use managed portrait cover/poster art or square album art—never cinematic landscape backgrounds—and include only owned canonical eligible credits with all eligible roles per work. Standard collection details resolve membership through the same catalog rules as the Collections surface. Listen playlists remain specialized lane-local queue and editing surfaces.
