...
- **Unified details:** Media, people, series, and standard collections open on canonical full-width `/details/...` routes. On desktop the shared detail stage targets 70% of the visible viewport, capped at 760 px, and combines identity artwork with the most useful owned content for that entity: tracks, chapters, episodes, sequence entries, collection items, or a person's owned works. Long audio lists scroll inside the stage and never overlap the navigation at its bottom. Overview combines the full description or biography, a cast/credits peek, series or collection context, and related content. Person works include only owned library media from canonical eligible credits and retain all of the person's roles per work. Listen playlists remain specialized lane-local queue and editing surfaces.
