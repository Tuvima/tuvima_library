...
- **One dashboard:** Home, media lanes, Search, detail pages, Settings/Admin, and Review Queue work together instead of forcing every correction into a separate management workspace. TV episodes stay inside their season on the show detail page, and each owned episode also has a show-scoped detail page using its still, synopsis, genre, and episode-aware Watch/Resume label. Show heroes report owned episodes and start from the first owned episode or resume the episode in progress. Continue cards retain the episode still and playback target, identify the child as `Sx Ey`, and keep Resume separate from Details.
