...
- **One dashboard:** Home, media lanes, Search, detail pages, Settings/Admin, and Review Queue work together instead of forcing every correction into a separate management workspace. TV show and episode detail pages share the same season selector and list only episodes present in the library; each owned episode has a show-scoped detail page using its still, synopsis, genre, and episode-aware Watch/Resume label. Show heroes report owned episodes and start from the first owned episode or resume the episode in progress. Continue cards retain the episode still and playback target, identify the child as `Sx Ey`, and keep Resume separate from Details.
