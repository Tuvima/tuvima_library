...
- **Unified details:** Media, people, series, and standard collections open on canonical full-width `/details/...` routes. On desktop the cinematic hero is exactly 70% of the visible viewport with no fixed-pixel maximum. Album tracks and audiobook chapters share that measured hero with identity and a bounded scrolling list; their full-width navigation follows without overlap. Structural details reserve the complete 70% hero for cinematic identity, then render full-width navigation followed by the sequence, owned-works, or collection array. Overview follows with the full description or biography, a purpose-built cast/credits peek, applicable series or collection context, and related content. Long sequences keep named Jump to options; alternate series/arc selectors and Show missing appear only when their data exists. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane. A person's works use managed portrait cover/poster art or square album art—never cinematic landscape backgrounds—and include only owned canonical eligible credits with all eligible roles per work. Listen playlists remain specialized lane-local queue and editing surfaces.
