...
- **Read**, **Watch**, and **Listen** are the media lanes where users browse and experience their library. Their Discover landings use `CinematicHeroCarousel` plus `SurfaceNavigationBar`; detail pages reuse that same lower navigation bar through `DetailTabs`. `/read/books`, `/read/comics`, `/watch/movies`, and `/watch/tv` keep the detailed browse shell. Listen keeps its permanent navigation rail: `/listen` is its row-based Discover surface, while `/listen/music` opens the tiled, filterable album browse and `/listen/audiobooks` opens the tiled audiobook browse. Music results are album-first; tracks remain available through Albums/Songs/Artists shortcuts and album/detail content.
