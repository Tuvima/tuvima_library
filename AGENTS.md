...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. The desktop cinematic stage occupies `95svh`, exposing its lower navigation while minimizing crop of native 16:9 artwork, and its edge-to-edge, top-center landscape backdrop extends beneath both the translucent global app bar and the translucent `SurfaceNavigationBar` at the hero's foot. A strong bottom fade protects text and hands the page off to following content; a restrained top fade protects global navigation and a left scrim protects the identity column. Audio details compose identity and a bounded internally scrolling, borderless track/chapter module inside that stage. Music albums are the deliberate exception to the lower detail-navigation pattern: the cover becomes a blurred full-stage atmosphere behind a larger sharp cover anchored high on the right, Play and Shuffle share the primary row, utility actions stay on the second row, and the album's Tracks/Credits/Details selector is local to the bounded workspace rather than a shared `DetailTab` shelf. The narrower track list has larger row type and no enclosing card, border, or opaque background, so it reads as part of the blurred hero; it has no search field, while Show missing remains available. An optional **More by** block shows at most three other owned albums linked through the exact canonical primary-artist credit and collapses completely when none qualify. Music albums render no shared `SurfaceNavigationBar` and no lower detail body. Album tracks and audiobook chapters are embedded content, not separate shared detail tabs; Editions is retired from all detail navigation. Audiobooks retain their existing detail navigation, chapter progress, direct chapter starts, bookmarks, speed, history, sleep, and other audiobook playback tools, gain a larger cover in otherwise unused stage space, and do not gain a video-style whole-book Restart pairing. Structural details reserve the remaining stage for identity, then render the ordered sequence, collection, or owned-works array below it on Overview only. Standard collections use the neutral cinematic wash with no member-derived backdrop and show up to four representative owned artworks through the same `MediaArtworkGroupPreview` cluster used by collection tiles at a dominant hero scale. Their hero has no Open, Shuffle, or lane action buttons; a compact factual row shows the owned year range, total item count, and applicable Read/Watch/Listen counts. Book and comic covers share the same visual-height envelope, and people use the larger portrait envelope. People render one continuous page with no detail tab bar. No region overlaps or clips another. Applicable selectors align left, Show missing and mixed-lane filters align right, and Jump to sits beside the array heading only above ten items. Every structural series array ends with a small owned summary and, only when the total is authoritative, missing/total text plus a simple progress bar. Overview follows the structural array and recommendations use a separate Related tab. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane, never ordinary series. A person's works use managed portrait cover/poster art or square album art rather than cinematic landscape backgrounds. Standard collection details resolve membership through the same catalog rules as the Collections surface. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
