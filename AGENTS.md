...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. Its desktop cinematic hero occupies exactly 70svh with no fixed-pixel maximum. Audio details compose identity and a bounded internally scrolling track/chapter module inside that measured hero, followed by the full-width `SurfaceNavigationBar`. Structural details reserve the complete 70svh hero for identity, then render the full-width navigation and ordered sequence, collection, or owned-works array below it on Overview only. No region overlaps or clips another. Applicable selectors align left, Show missing and mixed-lane filters align right, and Jump to sits beside the array heading only above ten items. Overview follows the structural array and recommendations use a separate Related tab. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane, never ordinary series. A person's works use managed portrait cover/poster art or square album art rather than cinematic landscape backgrounds. Standard collection details resolve membership through the same catalog rules as the Collections surface. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
...
- Detail navigation follows the complete 70svh identity hero on structural pages and follows the bounded embedded audio module on Listen pages. Structural arrays use their own full-width surface below navigation and render only with Overview; Overview combines the full attributed description or biography, a purpose-built role-aware cast/credits peek, and series or collection context when applicable. Related content belongs in its own tab. Do not repeat the primary array in other tabs or Details. Album tracks and audiobook chapters remain bounded, internally scrollable lists with sticky controls so the navigation shelf is always separate and unobscured.
