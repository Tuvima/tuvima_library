...
- Series and collection containers use the dedicated fixed-size landscape `MediaGroupTile`, which composes artwork from owned child items and reveals an in-place child carousel on hover or keyboard focus. The card never grows or changes shelf geometry. Its resting action opens the group; the carousel action opens the selected owned child using `DisplayCardPreviewItemDto.WebUrl`. Count/shape-aware resting layouts belong in `MediaArtworkGroupPreview`; interactive tile and future detail-page artwork browsing share the parent-controlled `MediaArtworkCarousel`. Individual media, Continue, search, and other existing tiles remain on `MediaTile` or their purpose-built renderer.
