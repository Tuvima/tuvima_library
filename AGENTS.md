...
- **Pre-beta destructive cutover rule:** until the product owner explicitly changes this rule, development database and configuration state may be deleted, reset, or rebuilt to reach the current target architecture. Do not add backups, data-preserving migrations, compatibility readers/writers, route aliases, dual schemas, or legacy fallbacks for pre-beta state. Fail fast on obsolete state and reingest from the configured source media. This rule applies only to disposable application/configuration state; existing/read-only media sources and user-owned originals must still be protected from unintended moves, renames, writeback, overwrite, or deletion.
