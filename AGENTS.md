...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. Its desktop `DetailStage` occupies 70% of the small viewport, capped at 760 px, and combines the shared cinematic identity with one Engine-selected primary module before placing `SurfaceNavigationBar` at the bottom edge. Tracks, chapters, episodes, sequences, collection items, and a person's owned works scroll within the stage rather than increasing or overlapping its height. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
