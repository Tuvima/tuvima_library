...
3. `src/MediaEngine.Web/Components/Cinematic/CinematicHeroCarousel.razor`, `CinematicHeroSurface.razor`, and `SurfaceTabBar.razor` for shared landing/detail cinematic presentation and filters/tabs.
...
- Landing pages must compose `CinematicHeroSurface`/`CinematicHeroCarousel` and `SurfaceTabBar`; do not create a second landing-only hero or tab renderer. Keep full item detail behavior in `DetailPage`, `DetailHero`, and their existing routes unchanged when evolving landing composition.
