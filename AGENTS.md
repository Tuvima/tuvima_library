...
- Non-TV series and collection containers use the dedicated fixed-size landscape `MediaGroupTile`, which composes artwork from owned child items and reveals an in-place child carousel on hover or keyboard focus. TV shows are the exception: they use the standard `MediaTile` identity treatment, showing the show cover at rest and expanding to the show-level cinematic backdrop, logo, facts, description, and actions on hover. A TV show must not receive the generic collection hover merely because it is structurally series-backed. The group card is approximately 70% of the original expanded-media footprint and never grows or changes shelf geometry. Its top line identifies the group; its selected child drives title, description, and media-specific facts. Its resting action opens the group; the carousel action opens the selected owned child using `DisplayCardPreviewItemDto.WebUrl`. Count/shape-aware resting layouts belong in `MediaArtworkGroupPreview`; interactive tile and future detail-page artwork browsing share the parent-controlled `MediaArtworkCarousel`. Shelf snap and stable-height logic must include both `.media-tile` and `.media-group-tile`. Individual media, TV shows, Continue, search, and other existing tiles remain on `MediaTile` or their purpose-built renderer.
...
- Provider sequence manifests use stable source container/member IDs and retain named missing entries. Container options merge only through shared identity evidence, never normalized display titles. Series detail shows the actual Series Set title, a selector only when alternate sets exist, a scoped Series selector only when multiple groups exist, and a Jump to selector when the active group has more than five entries.
