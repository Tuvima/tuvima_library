...
- The media Context Rail replaces generic Contents navigation in the normal editor. Keep the real media hierarchy and same-shell target switching; retain explicit audiobook chapter-title editing only under Chapters. Retail Match may change provider identity and structural placement together; preview previous/target paths before Apply when placement changes. Do not expose Change Type in the normal editor or Review Queue; low-level APIs are not a user-facing affordance.
- Universe/entity editing remains content inside `SharedMediaEditorShell`, reachable from an authorized media editor when a Universe QID exists. Preserve the original media target, use the existing dirty Save/Discard/Stay guard, and allow Universe → ordered category selector → entity → Universe root → original media in the same shell. Consumer Explore remains distinct.
