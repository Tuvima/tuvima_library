...
- **Read**, **Watch**, and **Listen** are the media lanes where users browse and experience their library. Their lane landings use `CinematicHeroCarousel` plus `SurfaceNavigationBar`; detail pages reuse that same lower navigation bar through `DetailTabs`. `/read/books`, `/read/comics`, `/watch/movies`, and `/watch/tv` keep the detailed browse shell. Listen keeps its permanent navigation rail on `/listen`, uses the compact square-art hero density, represents music rows as albums rather than tracks, and shows new arrivals as album cards under **New tracks added**; tracks remain available as album preview/detail content.
...
- Non-TV series and collection containers use the dedicated fixed-size landscape `MediaGroupTile`. Rest is artwork-only: two, three, or four representative owned images fill the card through approved count-and-shape templates selected from actual portrait, square, or wide artwork metadata. Images retain their natural ratios, the card never grows or changes shelf geometry, and more than four children are never loaded or rendered. Hover or keyboard focus keeps the exact composition and dimensions, adds the purple boundary/glow, and fades in only a compact top overlay with container type, title, and one truthful status line. The whole card is one group link. TV shows are the exception: they use the standard `MediaTile` identity treatment, showing the show cover at rest and expanding to the show-level cinematic backdrop, logo, facts, and description on hover. A TV show must not receive the generic collection hover merely because it is structurally series-backed. Group hovers never become carousels, rotate child artwork, or provide child-level navigation. Static representative layouts belong in `MediaArtworkGroupPreview`; the retired `MediaArtworkCarousel` and rotating collage must not be restored. Shelf snap and stable-height logic must include both `.media-tile` and `.media-group-tile`. Individual media, TV shows, Continue, search, and other existing tiles remain on `MediaTile` or their purpose-built renderer.
- Every media or group card is one semantic navigation target that opens its detail surface. Cards do not expose inline Read, Play, Listen, My List, reaction, remove, or details buttons. Book and comic portrait cards keep their exact resting cover geometry on hover or keyboard focus and communicate selection only with a stronger purple glow—no hover text or scrim. Square music and audiobook covers also preserve their geometry and may use a compact identity strip. Movie and TV cards may retain cinematic landscape expansion when a suitable background exists, but their left-aligned rating, classification, year, and runtime row uses only a compact translucent backing that hugs the text; they remain action-free.
...
3. `src/MediaEngine.Web/Components/Cinematic/CinematicHeroCarousel.razor`, `CinematicHeroSurface.razor`, and `SurfaceNavigationBar.razor`, plus `src/MediaEngine.Web/Components/Details/DetailHeroContent.razor`, for shared landing/detail cinematic presentation and navigation.
...
- Landing pages must compose `CinematicHeroSurface`/`CinematicHeroCarousel` with `DetailHeroContent`; do not create a landing-only identity, facts, action, progress, or synopsis renderer. Lane and detail lower navigation must use `SurfaceNavigationBar`. Group-opening actions such as **Open Show** use an open affordance, while progressed slides use the lane-specific Continue label and Resume action. Keep item-specific detail route state and content unchanged when evolving landing composition.
