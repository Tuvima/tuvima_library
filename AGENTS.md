...
- **View** is the personal mixed-local-media surface at `/view`. Personal View libraries index images, short videos, documents, and audio using local metadata, deduplicate content while preserving source paths, and support gallery/timeline browsing, search, favorites, hidden items, compound files, thumbnails, and available file/capture details. View assets never enter catalogue identity, provider, Wikidata, canonical-claim, or Review Queue workflows.
