...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. Its desktop cinematic hero occupies exactly 70svh with no fixed-pixel maximum. Audio details compose identity and a bounded internally scrolling track/chapter module inside that measured hero, followed by the full-width `SurfaceNavigationBar`. Structural details reserve the complete 70svh hero for identity, then render the full-width navigation and ordered sequence, collection, or owned-works array below it. No region overlaps or clips another. Overview follows the structural array. Long sequences preserve named Jump to options; alternate series/arc selectors and Show missing render only when applicable. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane, never ordinary series. A person's works use managed portrait cover/poster art or square album art rather than cinematic landscape backgrounds. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
...
- Detail navigation follows the complete 70svh identity hero on structural pages and follows the bounded embedded audio module on Listen pages. Structural arrays use their own full-width surface below navigation; Overview follows that surface and combines the full attributed description or biography, a purpose-built role-aware cast/credits peek, series or collection context when applicable, and related content. Do not repeat the primary array in Overview or Details. Album tracks and audiobook chapters remain bounded, internally scrollable lists with sticky controls so the navigation shelf is always separate and unobscured.
