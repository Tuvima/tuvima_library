...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. Its desktop `DetailStage` occupies exactly 70svh with no fixed-pixel maximum. Audio details compose `identity → bounded internally scrolling tracks/chapters → SurfaceNavigationBar`; structural details compose `identity → SurfaceNavigationBar → ordered sequence, collection, or owned-works array`. Both arrangements remain inside the same stage without overlap or clipped menus, and Overview begins after it. Long sequences preserve named Jump to options; alternate series/arc selectors and Show missing render only when applicable. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane, never ordinary series. A person's works use managed portrait cover/poster art or square album art rather than cinematic landscape backgrounds. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
...
- Detail navigation follows the identity on structural pages and follows the embedded audio module on Listen pages. The structural array then fills the remainder of the exact 70svh stage; Overview follows the stage and combines the full attributed description or biography, a role-aware cast/credits peek, series or collection context when applicable, and related content. Do not repeat the primary array in Overview or Details. Album tracks and audiobook chapters remain bounded, internally scrollable lists with sticky controls so the navigation shelf is always separate and unobscured.
