...
- **Operations** is one Administration destination with paired **Ingestion** and **Activity & Audit** views. Ingestion at `/settings/ingestion` is the live operational summary: current run state, truthful phase-specific progress, grouped active/queued work, current-run outcomes, media mix, provider waits, and real scheduled refreshes. File completion must never be presented as whole-run completion while required identity, enrichment, or organization operations remain. The screen refreshes automatically; its single **Scan now** action explicitly starts an extra scan of watched folders. On mobile it uses the same state in a compact summary with expandable stages. The navbar activity indicator opens Ingestion for ingestion, identity, and enrichment activity.
- **Activity & Audit** at `/settings/activity` is the detailed history paired with Ingestion. It has one operation-first view: named batches and administrative jobs lead to meaningful summary categories, category selection reveals chronological individual records, and each record can disclose technical details. There is no separate page-level timeline mode. Interrupted ingestion resumes under the same durable batch ID so one logical run remains one Activity entry. Mobile uses compact operation cards and vertical event cards. Activity uses the same run identity and outcome semantics as Ingestion.
