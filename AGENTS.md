...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. Its desktop `DetailStage` occupies 70% of the small viewport, capped at 760 px. Album tracks and audiobook chapters are the only primary modules embedded between the shared cinematic identity and the bottom `SurfaceNavigationBar`; those audio lists scroll internally without overlapping either region. Episodes, ordered series sequences, collection items, appearances, and a person's owned works render as complete natural-height arrays below the stage and before Overview. Portrait series/book/comic covers remain restrained inside the larger hero, and long sequences keep the existing named Jump to selector. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
...
- Detail navigation follows the identity or embedded audio module at the bottom of the 70% stage. Structural primary arrays follow the stage; Overview follows those arrays and combines the full attributed description or biography, a role-aware cast/credits peek, series or collection context when applicable, and related content. Do not repeat the primary array in Overview or Details. Album tracks and audiobook chapters remain bounded, internally scrollable lists with sticky controls so the navigation shelf is always separate and unobscured.
