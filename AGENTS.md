...
- **Detail pages** are where users view an item and fix/edit that item inline. Media, series, standard collections, and people use the canonical full-width `/details/{entityType}/{id}` surface. The desktop cinematic stage occupies one full visible viewport (`100svh`) and its top-anchored landscape backdrop extends beneath both the translucent global app bar and the translucent `SurfaceNavigationBar` at the hero's foot. A strong bottom fade protects text and hands the page off to following content; a restrained top fade protects global navigation and a left scrim protects the identity column. Audio details compose identity and a bounded internally scrolling, borderless track/chapter module inside that stage. Album tracks and audiobook chapters are embedded content, not separate detail tabs; music also omits Editions. Audiobooks retain chapter progress, direct chapter starts, bookmarks, speed, history, sleep, and other audiobook playback tools, but do not gain a video-style whole-book Restart pairing. Structural details reserve the remaining stage for identity, then render the ordered sequence, collection, or owned-works array below it on Overview only. Standard collections use this same hero and show up to four representative owned artworks with the shared stacked-art mechanics. Book and comic covers share the same visual-height envelope, and people use the larger portrait envelope. No region overlaps or clips another. Applicable selectors align left, Show missing and mixed-lane filters align right, and Jump to sits beside the array heading only above ten items. Overview follows the structural array and recommendations use a separate Related tab. Read/Watch/Listen filters belong only to mixed-media collections and people with owned works in more than one lane, never ordinary series. A person's works use managed portrait cover/poster art or square album art rather than cinematic landscape backgrounds. Standard collection details resolve membership through the same catalog rules as the Collections surface. Playlists remain specialized Listen queue/edit surfaces and do not use the shared detail route.
...
- Detail navigation occupies a translucent shelf at the foot of the complete `100svh` cinematic stage so the backdrop continues visibly beneath it. Structural arrays use their own full-width surface below navigation and render only with Overview; Overview combines the full attributed description or biography, a purpose-built role-aware cast/credits peek, and series or collection context when applicable. Related content belongs in its own tab. Do not repeat the primary array in other tabs or Details. Album tracks and audiobook chapters remain bounded, internally scrollable, borderless embedded lists with sticky controls so the navigation shelf stays separate and unobscured; do not restore Tracks or Chapters tabs, album Editions, Play All, or a duplicate My List control inside the list.
