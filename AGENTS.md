...
- **View** is the personal mixed-local-media surface at `/view`. Each enabled profile owns one Personal Space backed by an internal `personal` library bridge; any number of sources/devices may feed that space without becoming separate libraries. The standard View shell has Photos (`/view`), Folders (`/view/folders`), Galleries (`/view/galleries`), People (`/view/people`), and Places (`/view/places`). Folders preserves source and nested-folder identity with URL-backed breadcrumbs, including a first-class Family Library root in Shared scope. Browser uploads use `Timeline/<year>/<month>/<date-time><type>.<ext>`; additional linked or managed folders preserve their hierarchy and opt into Photos independently. Shared/Mine/Profile scope is resolved from trusted profile identity; View access, Shared contribution, and Gallery sharing are independent administrator policies. View indexes images, short videos, documents, and audio from local metadata, deduplicates physical content while preserving ownership and source paths, and supports cursor timeline/search, reversible personal states, compound files, thumbnails, Manual/Smart Galleries, and available file/capture details. Moving managed items to the Family Library creates and verifies every destination under `Shared` before deleting personal occurrences; linked read-only items are copied. Durable transfer state distinguishes complete moves from cleanup pending. Collections may reference a whole Gallery or a versioned View rule, never individual local asset IDs, and must reapply View authorization. View assets never enter catalogue identity, provider, Wikidata, canonical-claim, or Review Queue workflows. People and Places must report only real provenance-aware annotations/GPS metadata; face recognition, object/scene detection, OCR, semantic search, maps, public links, and mobile backup/sync remain unimplemented until backed by real clients/processors.
