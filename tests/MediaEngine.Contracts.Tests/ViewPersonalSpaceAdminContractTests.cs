...
{
    [Fact]
    public void ReviewContract_ExposesStatusWithoutStorageKeysOrDeviceIdentifiers()
    {
        var now = DateTimeOffset.Parse("2026-08-23T12:00:00Z");
        var response = new ViewPersonalSpaceAdminReviewDto(
            Guid.NewGuid(),
            new ViewPersonalSpaceAdminDto(Guid.NewGuid(), now.AddDays(-2), now),
            [new ViewSourceAdminDto(Guid.NewGuid(), "folder", "Family imports", now, now.AddDays(-2), now)],
