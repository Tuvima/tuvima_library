...
public sealed class PlaybackPrimitiveTests
{
    ...
    public void ListenSurfaces_UseSharedTransportControls()
    {
        ...
        var historyRow = File.ReadAllText(Path.Combine(root, "src/MediaEngine.Web/Components/Shared/PlaybackHistoryRow.razor"));
        var historyRowStyles = File.ReadAllText(Path.Combine(root, "src/MediaEngine.Web/Components/Shared/PlaybackHistoryRow.razor.css"));
        ...
        Assert.Contains("<PlaybackHistoryRow", panel, StringComparison.Ordinal);
        Assert.Contains("<PlaybackHistoryRow", popup, StringComparison.Ordinal);
        Assert.Contains("<PlaybackHistoryRow", bar, StringComparison.Ordinal);
        ...
        Assert.Contains("playback-history-row", historyRow, StringComparison.Ordinal);
        Assert.Contains("[Parameter(CaptureUnmatchedValues = true)]", historyRow, StringComparison.Ordinal);
        Assert.Contains("@attributes=\"AdditionalAttributes\"", historyRow, StringComparison.Ordinal);
        Assert.Contains("Icons.Material.Outlined.CalendarMonth", historyRow, StringComparison.Ordinal);
        Assert.Contains("playback-history-row__action-ring", historyRowStyles, StringComparison.Ordinal);
        Assert.Contains("var(--listen-accent, var(--listen-audio-accent, var(--tl-status-warning, #f59e0b)))", historyRowStyles, StringComparison.Ordinal);
