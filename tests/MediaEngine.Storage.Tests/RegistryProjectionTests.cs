...
public sealed class RegistryProjectionTests : IDisposable
{
    private readonly string _dbPath;
    private readonly DatabaseConnection _db;

    public RegistryProjectionTests()
    {
        DapperConfiguration.Configure();
        _dbPath = Path.Combine(Path.GetTempPath(), $"tuvima_registry_projection_{Guid.NewGuid():N}.db");
    ...
    [Fact]
    public async Task RegistryProjection_AppliesVaultGateAndNormalizedOverviewCounts()
    {
        ...
        var page = await repo.GetPageAsync(new RegistryQuery());
    ...
    [Fact]
    public async Task RegistryProjection_RequiresArtworkSettlementBeforeMissingCoverIsVisible()
    {
        ...
        var beforePage = await repo.GetPageAsync(new RegistryQuery());
