...
Canonical media, person, series, and standard-collection details use a full-width desktop stage that is exactly 70svh with no fixed-pixel maximum. Audio details place the bounded, internally scrolling tracks or chapters between identity and unobscured navigation. Structural details place navigation between identity and their ordered sequence, collection, appearance, or owned-works array; that array fills the remainder of the same stage before Overview begins. Long sequences retain named Jump to options and expose alternate series/arc or missing-item controls only when applicable. Read/Watch/Listen filters are reserved for mixed-media collections and people with owned works in multiple lanes. Person works use managed portrait covers/posters or square album art, never cinematic landscape backgrounds. Overview combines attributed description or biography, cast/credits, applicable series or collection context, and related content. Listen playlists remain specialized lane-local surfaces.
