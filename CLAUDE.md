...
Canonical media, person, series, and standard-collection details use a full-width desktop cinematic hero that is exactly 70svh with no fixed-pixel maximum. Audio details place bounded, internally scrolling tracks or chapters inside that hero after identity, with unobscured full-width navigation below. Structural details reserve the complete hero for identity, then place full-width navigation before their ordered sequence, collection, appearance, or owned-works array. Long sequences retain named Jump to options and expose alternate series/arc or missing-item controls only when applicable. Read/Watch/Listen filters are reserved for mixed-media collections and people with owned works in multiple lanes. Person works use managed portrait covers/posters or square album art, never cinematic landscape backgrounds. Overview follows the structural surface and combines attributed description or biography, a purpose-built cast/credits peek, applicable series or collection context, and related content. Listen playlists remain specialized lane-local surfaces.
