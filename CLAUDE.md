...
Canonical media, person, series, and standard-collection details use a full-width `100svh` desktop cinematic stage. Top-anchored landscape art extends beneath the translucent global app bar and translucent lower navigation; the bottom is deliberately darkened more heavily than the top so the source's upper composition and hero copy remain legible. Audio details place bounded, internally scrolling, borderless tracks or chapters inside that stage after identity, with unobscured navigation at its foot. Tracks, Chapters, and music Editions are not redundant tabs, and the embedded lists contain neither Play All nor a second My List action. Audiobooks retain chapter-level playback and their specialized player tools without receiving the video Resume/Restart pair. Structural details place their ordered sequence, collection, appearance, or owned-works array below the stage on Overview only. Collections use the shared hero with up to four stacked representative artworks; books and comics share one cover height, while people use the larger portrait height. Utility actions use icon-left-of-label controls on a left-growing row beneath the primary action. Jump to appears beside the array heading only above ten entries; alternate series/arc selectors align left and Show missing aligns right. Read/Watch/Listen filters are reserved for mixed-media collections and people with owned works in multiple lanes. Person works use managed portrait covers/posters or square album art, never cinematic landscape backgrounds. Overview combines attributed description or biography, a purpose-built cast/credits peek, and applicable series or collection context; recommendations use a separate Related tab. Standard collection details use the same resolved membership as the Collections catalog. Listen playlists remain specialized lane-local surfaces.
