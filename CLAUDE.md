...
Home alone uses `CinematicHeroCarousel`/`CinematicHeroSurface` as a landing treatment; detail heroes continue sharing that cinematic implementation through `DetailHeroContent`. Home has no All/Read/Watch/Listen submenu: its shelves overlap the full-viewport hero's bottom fade. The non-control slide background opens details, while the primary media action starts playback or reading. Read, Watch, and Listen use persistent rails with compact row-based Discover surfaces, and Collections opens directly as a filterable grid. Each media lane keeps one compact route-based Discover/media navigation bar mounted at the top while the content below changes; redundant lane identity and sidebar lane-name blocks are omitted. Desktop rails are viewport-anchored and do not move when their adjacent media content panes scroll. Selecting Books, Comics, Movies, TV, Series, Music, or Audiobooks immediately opens the full tiled browse and its filters beneath that menu, with no second **View all** step or duplicate media switcher. Direct filters are unboxed page content with larger shared typography: search occupies the first row; genre, creator, and year share a searchable multi-select checklist component; boolean filters stay as native checkboxes; and tile-size plus Cards/List are right aligned. The series checkbox changes results without changing the control surface. Recommendation shelves retain **View all**. Listen remains album-first, while Songs uses the shared audio table with artwork, play/queue controls, drag-to-playlist, resizable columns, and profile-scoped qualified play counts.

Canonical media, person, series, and standard-collection details use a full-width `95svh` desktop cinematic stage so its lower navigation remains visible while native 16:9 artwork loses as little of the frame as possible. Edge-to-edge landscape art is pinned top-center beneath the translucent global app bar and translucent lower navigation; the bottom is deliberately darkened more heavily than the top so the source's upper composition and hero copy remain legible. Music albums use an album-only hero workspace: the cover supplies a blurred full-stage atmosphere and a larger sharp cover high on the right; Play and Shuffle occupy the primary row; utility actions sit below; and a local Tracks/Credits/Details selector controls one bounded workspace. The track list is deliberately narrower with larger type and no card border or opaque panel, retains Show missing, and omits search so it remains visually embedded in the atmosphere. This is not a shared detail-tab shelf: MusicAlbum has no shared lower navigation or lower body. Its optional **More by** area shows at most three other owned albums from the exact canonical primary artist and disappears when empty. Audiobooks retain the shared detail structure, chapter-level playback, and specialized player tools, use a larger cover, and do not receive the video Resume/Restart pair. Album tracks and audiobook chapters remain embedded; Tracks, Chapters, and Editions do not return to shared navigation, and the lists contain neither Play All nor a second My List action. Structural details place their ordered sequence, collection, appearance, or owned-works array below the stage on Overview only, ending with an owned summary and an authoritative-total progress bar when available. Collections use a neutral no-backdrop hero and the same up-to-four representative artwork cluster as collection tiles at a dominant scale; there are no collection action buttons, and a compact row reports the year range, total items, and applicable Read/Watch/Listen counts. Books and comics share one cover height, while people use the larger portrait height and one continuous page without tabs. Utility actions use larger icon-left-of-label controls on a left-growing row beneath the primary action. Jump to appears beside the array heading only above ten entries; alternate series/arc selectors align left and Show missing aligns right. Read/Watch/Listen filters are reserved for mixed-media collections and people with owned works in multiple lanes. Person works use managed portrait covers/posters or square album art, never cinematic landscape backgrounds. Overview combines attributed description or biography, a purpose-built cast/credits peek, and applicable series or collection context; recommendations use a separate Related tab. Standard collection details use the same resolved membership as the Collections catalog. Listen playlists remain specialized lane-local surfaces.
