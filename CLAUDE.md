...
Home alone uses `CinematicHeroCarousel`/`CinematicHeroSurface` as a landing treatment; detail heroes continue sharing that cinematic implementation through `DetailHeroContent`, and both surfaces use `HeroBackdrop` as the single owner of foreground cover/album geometry and book page/spine decoration. Home has no All/Read/Watch/Listen submenu: its shelves overlap the full-viewport hero's bottom fade. The non-control slide background opens details, while the primary media action starts playback or reading. Read, Watch, Listen, and Collections use the shared persistent section shell with one compact route-based navigation bar and a viewport-anchored desktop rail. Collections routes Overview, Automatic, Curated, Shelves, and People through that component system: Curated is administrator-authored and library-published; Automatic requires trusted relationships connecting multiple lower-level groups; Shelves contains only contributor-based multi-work groupings (books by author, comics by creator/writer, movies by director, albums by primary artist, and audiobooks by author or narrator); People is a paged canonical-contributor list; and playlists remain in Listen. Albums, TV shows, and series/volumes stay lane-native and are not Collections shelves. Redundant lane identity and sidebar lane-name blocks are omitted. Selecting Books, Comics, Movies, TV, Series, Music, or Audiobooks immediately opens the full tiled browse and its filters beneath that menu, with no second **View all** step or duplicate media switcher. Direct filters are unboxed page content with larger shared typography: search occupies the first row; genre, creator, and year share a searchable multi-select checklist component; boolean filters stay as native checkboxes; and tile-size plus Cards/List are right aligned. The series checkbox changes results without changing the control surface. Recommendation shelves retain **View all**. Listen Discover defaults to larger tiles; direct Music browse retains its intentionally smaller default. Music Timeline uses the shared timeline renderer and contains albums only. Songs uses the shared audio table with artwork, play/queue controls, drag-to-playlist, resizable columns, and profile-scoped qualified play counts.
