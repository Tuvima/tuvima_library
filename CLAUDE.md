...
Canonical media, person, series, and standard-collection details use a full-width desktop cinematic hero that is exactly 70svh with no fixed-pixel maximum. Audio details place bounded, internally scrolling tracks or chapters inside that hero after identity, with unobscured full-width navigation below. Structural details reserve the complete hero for identity, then place full-width navigation before their ordered sequence, collection, appearance, or owned-works array on Overview only. Jump to appears beside the array heading only above ten entries; alternate series/arc selectors align left and Show missing aligns right. Read/Watch/Listen filters are reserved for mixed-media collections and people with owned works in multiple lanes. Person works use managed portrait covers/posters or square album art, never cinematic landscape backgrounds. Overview combines attributed description or biography, a purpose-built cast/credits peek, and applicable series or collection context; recommendations use a separate Related tab. Standard collection details use the same resolved membership as the Collections catalog. Listen playlists remain specialized lane-local surfaces.
