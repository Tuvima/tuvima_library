...
**Container card rule:** Non-TV series and collection containers use a fixed-size landscape card. The left identity stays aligned between rest and hover; the right side renders two to four representative owned covers in one row, shrinking each cover as the count grows. Hover or keyboard focus adds a semantic container summary without growing the card or wrapping text in another bordered panel. Summary rows use media counts, sequence ownership, and progress rather than repeating child titles. Both states open the container; cards do not contain buttons, rotate artwork, or expose child routes. Ordered series use structural representatives, while broader curated collections prefer media-type breadth. TV shows are rendered as rich media identities instead: show cover at rest and the show-level cinematic background, logo, facts, and description on hover; owned episodes stay available on the show detail surface.
