...
**Container card rule:** Non-TV series and collection containers use a fixed-size landscape card. Rest is artwork-only: two to four representative owned images use approved templates selected by visible count and actual portrait, square, or wide shape. Hover or keyboard focus keeps that exact composition and frame, adds the purple boundary/glow, and reveals only container type, title, and one truthful status line in a compact top overlay. Both states open the container; cards do not contain buttons, rotate artwork, or expose child routes. Ordered series use structural/progress-aware representatives, while broader curated collections prefer media-type breadth. TV shows are rendered as rich media identities instead: show cover at rest and the show-level cinematic background, logo, facts, and description on hover; owned episodes stay available on the show detail surface.
