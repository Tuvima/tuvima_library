...
- **Detail pages** use one canonical full-width surface for media, series, standard collections, and people. An exact 70svh desktop stage with no fixed-pixel maximum composes audio as identity/list/navigation and structural details as identity/navigation/array. Audio lists scroll internally; structural sequence, collection, appearance, and owned-person-work arrays fill the stage remainder without overlapping the hero or menus. Long sequences retain named Jump to options and show alternate set/arc or missing controls only when real data supports them. Read/Watch/Listen filters belong only to mixed-media collections and multi-lane people. Person works are owned-only canonical credits rendered with portrait cover/poster or square album art, never landscape backgrounds. Overview combines description/biography, credits or cast, series/collection context, and related content; playlists remain specialized Listen surfaces.
