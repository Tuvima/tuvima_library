...
Non-TV series and collection containers render through the dedicated fixed-size landscape `MediaGroupTile`. Rest is artwork-only: two to four representative images use approved count-and-shape templates and retain their natural portrait, square, or wide ratios. Hover or keyboard focus preserves the artwork and dimensions, adds the purple boundary/glow, and reveals only type, title, and one status line in a compact top overlay. The whole surface opens the group; there are no buttons, rotating artwork, carousel, or child-level actions. TV shows render through `MediaTile`, using the show cover at rest and the show-level cinematic backdrop and rich identity on hover. Every individual or group card is one details link. Book and comic portrait cards keep their resting cover geometry and add only a stronger purple glow, without hover text or a scrim. Square music and audiobook covers also keep their geometry and may use a compact identity strip. Movie and TV cards may retain cinematic expansion without inline actions; their left-aligned rating, classification, year, and runtime row uses a compact translucent backing that hugs the text. Individual and Continue cards keep their existing renderers.
