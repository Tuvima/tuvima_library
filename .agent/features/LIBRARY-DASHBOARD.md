...
- **Detail pages** use one canonical full-width surface for media, series, standard collections, and people. The desktop cinematic stage fills `95svh` so its lower navigation stays visible while retaining substantially more native 16:9 artwork; edge-to-edge landscape art is pinned top-center beneath translucent global and lower navigation, with its lower region darkened into the following content. Music albums use that shared stage with a cover-derived blurred atmosphere, a large sharp album/record composition centered in the right half, bottom-left identity, Play and Shuffle together, icon-left utilities below, and one shared **Tracks** navigation item. Below the hero, the restrained Tracks surface gives most width to a borderless track list with Show missing and no search, shows managed deduplicated contributor portraits and combined roles in the right rail, retains details/sources, and conditionally renders every other owned album from the exact canonical primary artist in a full-width **More by** shelf. Audiobooks retain their shared detail structure and chapter/player tools while using a substantially larger cover where viewport height permits. Album Tracks is the sole music-album tab; audiobook Chapters remain embedded, Editions stays retired, and lists do not repeat Play All or My List. Structural details render their sequence, collection, appearance, or owned-person-work array below the stage and end with a compact ownership summary plus authoritative-total progress when available. Collections have no member-derived backdrop: they use the neutral hero, a dominant version of the shared up-to-four collection-tile artwork cluster, no collection action buttons, and one compact row for year range, total items, and applicable Read/Watch/Listen counts. Comic covers match book size, person portraits are enlarged, people use one page without tabs, and larger icon-left utility controls grow from the left beneath the primary action. Long sequences retain named Jump to options and show alternate set/arc or missing controls only when real data supports them. Read/Watch/Listen filters belong only to mixed-media collections and multi-lane people. Person works are owned-only canonical credits rendered with portrait cover/poster or square album art, never landscape backgrounds. Overview follows the structural surface and combines description/biography, a purpose-built credits or cast peek, series/collection context, and related content; playlists remain specialized Listen surfaces.
