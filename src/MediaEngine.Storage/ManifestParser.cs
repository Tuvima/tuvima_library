...
/// <summary>
/// Reads and writes the legacy manifest using <see cref="System.Text.Json"/>.
/// No external serialisation library is required.
///
/// Recovery behaviour (spec: "MUST attempt to load from .bak before halting"):
///   1. Try the primary manifest file.
///   2. If missing/corrupt, try the <c>.bak</c> backup and restore primary.
...
public sealed class ManifestParser : IStorageManifest
{
    ...
    /// Full path to the legacy manifest file.
    ...
    /// </summary>
    private static LegacyManifest CreateDefaultManifest() => new()
    {
        SchemaVersion = "1.0",
    ...
    private static bool TryDeserialize(string path, out LegacyManifest? manifest)
    {
        manifest = null;
        ...
        try
        {
            ...
            manifest = JsonSerializer.Deserialize<LegacyManifest>(json, SerializerOptions);
