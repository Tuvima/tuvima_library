...
{
    public void RunPreSchemaCompatibilityMigrations(SqliteConnection conn)
    {
        MigrateMetadataProvidersTable(conn);
    }

    public void RunMigrations(SqliteConnection conn)
    {
    ...
    private static void MigrateUniversalHubSystem(SqliteConnection conn)
    {
        ...
        if (!hubsExist)
        {
            return; // Fresh DB with new schema.sql already has all columns
        }
