-- =============================================================================
-- Tuvima Library Ã¢â‚¬â€ SQLite Initialisation Script
-- Phase 4: Storage Schema & Persistent State
--
-- Conventions
--   * Internal UUIDs are stored as 16-byte BLOBs in RFC4122/network byte order.
--   * External identifiers stay TEXT (QIDs, provider item IDs, URLs, hashes).
--   Ã¢â‚¬Â¢ BOOLEAN is stored as INTEGER: 1 = true, 0 = false.
--   Ã¢â‚¬Â¢ DATETIME is stored as TEXT in ISO-8601 format (datetime('now') default).
--   Ã¢â‚¬Â¢ All CREATE statements are idempotent (IF NOT EXISTS).
--   Ã¢â‚¬Â¢ Foreign keys are enforced; enable them per-connection with PRAGMA.
-- =============================================================================
...
--   â€¢ An album is a Parent Work; its tracks are Child Works whose
--     parent_work_id points at the album row.
--   â€¢ A TV show is a Parent of seasons, which are themselves Parents
--     of episodes â€” three rows linked by parent_work_id.
--   â€¢ A standalone movie or single-volume novel has work_kind='standalone'
--     and parent_work_id=NULL.
--   â€¢ A Catalog row exists when Wikidata or a retail provider knows about
...
-- Every row here MUST be derivable from Ã¢â€°Â¥1 rows in metadata_claims
...
-- file moves (spec: Hash Dominance invariant Ã¢â‚¬â€œ enforced at application layer).
...
-- â”€â”€ FTS5 Search Index â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- for items like "AmÃ©lie" and "Le Fabuleux Destin d'AmÃ©lie Poulain".
--
