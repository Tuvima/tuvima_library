...
{
    Task<MediaEditorNavigationReadService.MediaEditorNavigatorEnvelope?> GetNavigatorAsync(
        Guid entityId,
...
{
    Task<IReadOnlyList<MediaEditorNavigationReadService.MembershipSuggestionEnvelope>> GetSuggestionsAsync(
        Guid entityId,
        string field,
        string? query,
        string? source,
    ...
    Task<MediaEditorNavigationReadService.MembershipPreviewEnvelope?> PreviewAsync(
        Guid entityId,
        MediaEditorNavigationReadService.MembershipPreviewRequest request,
        CancellationToken ct);

    Task<MediaEditorNavigationReadService.MembershipPreviewEnvelope?> ApplyAsync(
        Guid entityId,
        MediaEditorNavigationReadService.MembershipPreviewRequest request,
        CancellationToken ct);
}

public sealed class MediaEditorNavigationReadService(
    IDatabaseConnection db,
    ISearchService searchService,
    IHydrationPipelineService pipeline) : IMediaEditorNavigationReadService, IMediaEditorMembershipReadService
{
    public async Task<MediaEditorNavigatorEnvelope?> GetNavigatorAsync(Guid entityId, CancellationToken ct) =>
        await BuildMediaEditorNavigatorCoreAsync(entityId, db, ct);

    public async Task<IReadOnlyList<MembershipSuggestionEnvelope>> GetSuggestionsAsync(
        Guid entityId,
        string field,
        string? query,
        string? source,
        ...
        string? parentValue,
        CancellationToken ct) =>
        await BuildMembershipSuggestionsAsync(entityId, field, query, source, parentEntityId, parentValue, db, searchService, ct);

    public async Task<MembershipPreviewEnvelope?> PreviewAsync(Guid entityId, MembershipPreviewRequest request, CancellationToken ct) =>
        await BuildMembershipPreviewAsync(entityId, request, db, ct);

    public async Task<MembershipPreviewEnvelope?> ApplyAsync(Guid entityId, MembershipPreviewRequest request, CancellationToken ct) =>
        await ApplyMembershipChangeAsync(entityId, request, db, pipeline, ct);
    ...
    private static async Task<MediaEditorNavigatorEnvelope?> BuildMediaEditorNavigatorCoreAsync(
        Guid entityId,
        IDatabaseConnection db,
        CancellationToken ct)
    {
