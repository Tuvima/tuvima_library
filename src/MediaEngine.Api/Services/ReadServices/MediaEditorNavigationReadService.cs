...
{
    Task<object?> GetNavigatorAsync(Guid entityId, CancellationToken ct);
}

public interface IMediaEditorMembershipReadService
{
    Task<object> GetSuggestionsAsync(Guid entityId, string field, string? query, string? source, Guid? parentEntityId, string? parentValue, CancellationToken ct);
    Task<object?> PreviewAsync(Guid entityId, MediaEditorNavigationReadService.MembershipPreviewRequest request, CancellationToken ct);
    Task<object?> ApplyAsync(Guid entityId, MediaEditorNavigationReadService.MembershipPreviewRequest request, CancellationToken ct);
}
...
{
    public async Task<object?> GetNavigatorAsync(Guid entityId, CancellationToken ct) =>
        await BuildMediaEditorNavigatorCoreAsync(entityId, db, ct);

    public async Task<object> GetSuggestionsAsync(Guid entityId, string field, string? query, string? source, Guid? parentEntityId, string? parentValue, CancellationToken ct) =>
        await BuildMembershipSuggestionsAsync(entityId, field, query, source, parentEntityId, parentValue, db, searchService, ct);

    public async Task<object?> PreviewAsync(Guid entityId, MembershipPreviewRequest request, CancellationToken ct) =>
        await BuildMembershipPreviewAsync(entityId, request, db, ct);

    public async Task<object?> ApplyAsync(Guid entityId, MembershipPreviewRequest request, CancellationToken ct) =>
