...
| Settings > Media Management | Live | Administrators manage catalogued and personal libraries, stable source identities, incoming locations, presentation, metadata, ownership, visibility, intake, duplicate, and organization policies. Every source independently declares Managed by Tuvima or Existing library safety. | Reorganization execution remains an explicit dry-run-and-confirm operation. | Yes through Engine/config actions. | Yes. | Yes. | Add per-library operational history after beta feedback. |
| View | Live (MVP) | Personal libraries index mixed local images, short videos, documents, and audio without catalogue identity or external enrichment. View supports library-scoped timeline/gallery browsing, local search, favorites, hidden items, compound files, duplicate source paths, and original-file viewing. | Face/object/OCR search, maps, memories, remote sharing, and AI search are post-beta. | Yes for local flags and collections; originals follow source policy. | Yes, through `/api/v1/view/*`. | Yes. | Harden derivative cache lifecycle and connected-device intake during beta. |
