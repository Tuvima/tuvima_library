...
| Settings > Libraries | Live | Administrators can create, edit, and remove typed libraries, choose source paths, recursion, intake and read-only behavior, and select enriched, local-preferred, local-only, or manual metadata policy. | Some folder changes may still require a rescan depending on watcher state. | Yes through Engine/config actions. | Yes. | Yes. | Add per-library operational history after beta feedback. |
| Photos | Live (MVP) | A separate asset index scans photo libraries in place, groups by capture month, searches filenames, creates thumbnails, preserves duplicate source paths, and supports favorites, hidden items, albums, selection, EXIF camera/GPS display, and original-file viewing. | Face/object/OCR search, maps, memories, remote sharing, mobile sync, and broad RAW support are post-beta. | Yes for flags and albums; originals remain unchanged. | Yes, through `/api/v1/photos/*`. | Yes. | Harden cache lifecycle and file watching during beta; defer privacy-sensitive intelligence until post-beta. |
