...
If you manually lock a field in the Vault, your value wins at confidence 1.0. Full stop. No source — not even Wikidata — overrides a user lock.
...
When two claims for the same field are within 0.05 confidence of each other — close enough that neither is clearly better — the field is marked **Conflicted**. The Vault surfaces conflicted fields for your review rather than silently picking one.
