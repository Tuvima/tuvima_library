...
The Home page and media lane pages are for browsing — discovering what's in your library, finding something to read or watch. The **Vault** is different. It's the command centre for managing your library: seeing what the Engine has done, what needs your attention, and taking deliberate action.
...
Every file in your library. Books, audiobooks, TV episodes, films, music tracks, comics, podcasts — all visible in one place, filterable by media type using the chips in the pinned header.
...
Every author, narrator, director, composer, screenwriter, and illustrator connected to your library. Always Wikidata-sourced — you can't add people manually, because people data is maintained by Wikidata's community and updated on a 30-day refresh cycle.

Each person entry shows a photo (when available), a name, a description, role chips (what roles they appear in your library — Author, Director, Narrator, etc.), and library presence counts (how many Works they're connected to).
...
Smart collections, personal lists, AI-generated mixes, and user-created playlists. The Hubs tab is for oversight and configuration — enabling or disabling smart hubs, adjusting thresholds, seeing what's in each collection. See the separate documentation on Hubs and Playlists for the full detail.
...
On mobile, the sidebar becomes a slide-out drawer triggered by the filter icon — same content, different form factor.
...
### Provisional
The Engine couldn't match this item confidently against an external source. The file's embedded metadata is being used as-is. This is not an error — it's the Engine saying "I did my best, but I'm not certain."
...
- **Sync** — whether metadata writeback is enabled for this item, last sync timestamp
- **Enrichment** — which stages have run, when they last ran, next scheduled run
- **Pipeline** — stage-by-stage progress indicators (Scan, Retail, Wikidata), with inline resolution panels for each stage
- **File** — path, size, format, fingerprint hash, media asset ID
- **Claims** — the full claim history for every metadata field; who said what with what confidence

**Pinned action bar** (always visible at bottom):
- **Identify** — re-run the identification pipeline for this item
- **Sync Now** — immediately write resolved metadata back to the file's embedded tags
- **Purge** — remove this item from the library (confirmation required)
...
The Retail stage shows **"Unmatched"** (not a green "Completed") when only the file scanner matched and no retail provider confirmed the identity. This distinction matters — "Completed" means an external source confirmed the match. "Unmatched" means the file metadata is all you have.
...
- **Delete** — removes selected items with a confirmation dialog (lists what will be deleted)
- **Sync Now** — triggers immediate metadata writeback for all selected items
...
Clicking on a TV show, music album, or book series navigates to a **detail sub-page** — a richer view with cover art header, metadata, and the hierarchical content (season accordion for TV, flat track list for music). Breadcrumb navigation returns you to the Vault list.
...
- View the "Linked Identities" section — if an author uses a pen name, both identities are shown as connected via Wikidata's pseudonym resolution
...
Similar to People, the Universes tab is always Wikidata-sourced. You can't manually create a Universe — they emerge from the metadata relationships the Engine discovers.
...
- Click a Series name in the detail drawer — this navigates back to the Media tab with that Series's filter applied, showing you exactly which items belong to it
...
For technical details about the Vault's implementation — component architecture, sidebar state management, column definition format, and the full detail drawer section structure — see the [architecture deep-dive](../architecture/settings-and-vault.md).
