...
When a new file lands in one of your watched folders, Tuvima Library doesn't just copy it somewhere and call it done. It goes through a careful, multi-stage journey — from raw unknown file to fully identified, enriched, and organized library entry. This page walks you through every step of that journey and explains the reasoning behind each one.
...
File appears in watched folder
  → Settle (wait for copy to finish)
  → Lock check (confirm file is fully written)
  → Fingerprint (SHA-256 identity)
  → Scan (extract embedded metadata)
  → Classify (resolve ambiguous formats)
  → Identify (match against known works)
  → Stage (hold in .data/staging/)
  → Hydrate (enrich from external sources)
  → Promote (move to organized library location)
```
...
The Engine uses a FileSystemWatcher to monitor your configured library folders. The moment a file appears, it's noticed — but not immediately acted on.
...
The Engine computes a SHA-256 hash of the file's contents. This hash is the file's permanent identity — its fingerprint.
...
|---|---|---|
| EPUB | 100 | Ebooks — extracts OPF metadata, cover art, author, series |
| Audio | 95 | M4B, MP3 — reads ID3/iTunes tags, embedded artwork |
| Video | 90 | MKV, MP4 — reads container metadata, subtitle tracks |
| Comic | 85 | CBZ, CBR — reads ComicInfo.xml if present |
| Generic | fallback | Anything else — filename parsing only |

The processor extracts embedded metadata — title, author, year, cover art, series name, narrator, ISBN — and emits these as **Claims**. Each Claim carries a source and a confidence score. A title found in a well-structured EPUB OPF file gets higher confidence than one guessed from a messy filename. These Claims feed the Priority Cascade described in [how-scoring-works.md](how-scoring-works.md).
...
The Engine uses an AI component called the **MediaTypeAdvisor** to resolve this. It examines tag signals: the genre tag, whether a narrator field is present, whether an ASIN (Amazon's audiobook identifier) is embedded, and whether the duration is consistent with an audiobook (typically 30+ minutes per file) or a music track (3–5 minutes).
...
All files land in `.data/staging/{assetId12}/` first — every single one, regardless of confidence. This flat structure (one subfolder per asset, named by a 12-character asset ID) is the Engine's safe holding area.

The data store tracks each file's status. There are no subcategory folders for "pending" or "low-confidence" — the database handles all of that. The only special folder is `staging/rejected/` for files you've explicitly rejected.
...
Once staged, the file enters the two-stage enrichment pipeline. Stage 1 pulls cover art, descriptions, and ratings from retail providers. Stage 2 uses Wikidata to fetch canonical structured data — author, genre, series, franchise relationships, and person data.
...
Once a file is hydrated and its confidence meets the threshold (≥0.85), it's **promoted** from staging to its final organized location using a media-type-specific path template.
...
This is how the library handles multiple versions gracefully. The hardback and the paperback of the same novel, the theatrical cut and the director's cut of the same film, the standard MP3 and the lossless FLAC of the same album — all coexist as Editions under one Work, not as separate entries cluttering your library.
...
For technical details about the ingestion pipeline — queue management, folder configuration, path template syntax, and the full staging directory layout — see the [architecture deep-dive](../architecture/ingestion-pipeline.md).
