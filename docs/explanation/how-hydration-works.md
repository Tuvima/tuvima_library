...
Stage 1 runs retail providers in waterfall order — each provider is tried in sequence until a confident match is found, or all providers have been tried.
...
**Cover art** — the file's embedded artwork is compared to the provider's returned image using perceptual hashing (pHash). Similar images score higher as match evidence. This prevents the Engine from accepting a confident title match that returns an obviously wrong cover.

**Description and plot summary** — used both for display and as evidence in candidate ranking. A description mentioning the same characters as the file's embedded metadata is a stronger match signal.

**Ratings** — reader/critic scores from the provider, stored as Claim data.

**Bridge IDs** — this is the key output. Bridge IDs are the identifiers that cross-reference items between databases:
...
The lookup sequence:
1. Try ISBN → find the specific Wikidata edition item
...
- P577 (publication date — first publication, not reprint)
...
**Wikipedia descriptions** are also fetched via `GetWikipediaSummariesAsync` — the formatted introductory text from Wikipedia, which is rich narrative context the AI's Description Intelligence service can work with.
...
Any person name found in the metadata — from file tags, structured properties, or AI extraction — that doesn't yet have a companion Wikidata QID gets searched independently. The search:

1. Queries Wikidata using the name string
2. Filters results to Q5 (human) — no accidentally matching fictional characters or organizations
3. Scores candidates by occupation match (is this person an author? a film director?) and notable work alignment
4. Auto-accepts matches at ≥0.80 confidence (0.75 for AI-extracted names, which are less reliable)

Person freshness check: if a person has been enriched within the last 30 days, they skip re-fetch. For stale persons, the Engine checks `last_revision_id` first — if Wikidata hasn't changed since the last fetch, a full property re-fetch is skipped. This keeps person data current without hammering the Wikidata API.

Person directories under `.data/images/people/{QID}/` are created lazily — only when a headshot image actually exists. Empty directories are never created just to reserve a space.
...
**Group-level refresh** applies to TV series and music albums. When any episode in a TV series triggers a stale refresh, all sibling episodes in the same series are included in the same batch. This keeps series data consistent — if a show's season count is updated in Wikidata, all episodes get the updated data in the same refresh cycle rather than staggering out over days as individual episodes hit their 30-day mark.

---

For technical details about provider configuration, waterfall priority configuration, the Wikidata property fetch list, caching TTLs, and the full hydration pipeline implementation — see the [architecture deep-dive](../architecture/hydration-and-providers.md).
