...
**Processor:** `EpubProcessor` â€” priority 100
...
| `cover_image` | â€” | Extracted from EPUB manifest cover item |

### Providers

Waterfall order during Stage 1:

1. Open Library â€” ISBN-first lookup, description and cover
2. Google Books â€” ISBN-first, falls back to title+author search
3. Apple API â€” title+author search, cover art and ratings
4. Wikidata â€” Stage 2, QID resolution via ISBN bridge
...
|---|---|---|
| `.m4b` | MPEG-4 Audio Book | Unambiguous â€” always audiobook |
| `.mp3` | MPEG Audio Layer 3 | Ambiguous â€” may be music or audiobook |

### Extracted metadata

| Field | Confidence | Notes |
|---|---|---|
| `title` | 0.90 | From ID3 `TIT2` or M4B `Â©nam` tag |
| `author` | 0.90 | From ID3 `TPE1` or M4B `Â©ART` tag |
| `narrator` | 0.85 | From ID3 `TPE3` or M4B `Â©wrt` tag |
| `album` | 0.90 | Series name often embedded here |
| `year` | 0.90 | From ID3 `TDRC` |
| `genre` | 0.80 | From ID3 `TCON` |
| `track_number` | 0.95 | From ID3 `TRCK` â€” used for multi-part audiobooks |
| `duration_sec` | 1.0 | Computed from audio stream |
| `container` | 1.0 | `m4b` or `mp3` |
| `audio_bitrate` | 1.0 | From audio stream properties |
| `asin` | 1.0 | From ID3 custom tag `TXXX:ASIN` â€” used as retail bridge ID |
| `cover_image` | â€” | From embedded ID3 `APIC` frame |
...
1. Apple API â€” ASIN-first lookup for Audible content; cover art, narrator, series data
2. Wikidata â€” Stage 2, QID resolution
...
**Processor:** `VideoProcessor` â€” priority 90

### Supported formats

| Extension | Format | Ambiguity |
|---|---|---|
| `.mkv` | Matroska Video | Ambiguous â€” may be movie or TV |
| `.mp4` | MPEG-4 Video | Ambiguous â€” may be movie, TV, or other |
...
| Field | Confidence | Notes |
|---|---|---|
| `title` | 0.75 | From filename parsing (lower confidence â€” filenames vary widely) |
...
1. Filename matches `SxxExx` or `1x01` season/episode pattern â†’ TV
2. Filename contains year in parentheses `(2024)` and no episode markers â†’ Movie candidate
...
1. TMDB â€” title+year search; cover art (poster), description, ratings, cast, TMDB ID bridge
2. Wikidata â€” Stage 2, QID resolution via TMDB ID bridge
...
1. TMDB â€” series and episode metadata; poster and backdrop images
2. Wikidata â€” Stage 2, QID resolution
...
| Extension | Format | Ambiguity |
|---|---|---|
| `.flac` | Free Lossless Audio Codec | Unambiguous â€” always music |
| `.ogg` | Ogg Vorbis | Unambiguous â€” always music |
| `.wav` | Waveform Audio | Unambiguous â€” always music |
| `.mp3` | MPEG Audio Layer 3 | Ambiguous â€” see audiobooks |
| `.m4a` | MPEG-4 Audio | Ambiguous â€” may be music or audiobook |
...
| `musicbrainz_recording_id` | 1.0 | From `TXXX:MusicBrainz Track Id` â€” used as bridge ID |
...
1. MusicBrainz â€” recording and release lookup via MBID bridge; track, album, and artist metadata
2. Wikidata â€” Stage 2, QID resolution
...
**Processor:** `ComicProcessor` â€” priority 85
...
1. Metron (Comic Vine) â€” issue search by series+number; publisher, creator, and story arc data
2. Wikidata â€” Stage 2, QID resolution for notable series
...
|---|---|---|
| `.mp3` | Uncertain | Heuristics â†’ AI `MediaTypeAdvisor` |
| `.mp4` | Uncertain | Filename pattern â†’ AI `MediaTypeAdvisor` |
| `.m4a` | Uncertain | Heuristics â†’ AI `MediaTypeAdvisor` |
| `.m4v` | Movie | Filename pattern â†’ AI confirmation |
| `.mkv` | Movie or TV | Filename pattern â†’ AI confirmation |
| `.avi` | Movie or TV | Filename pattern â†’ AI confirmation |
| `.webm` | Movie or TV | Filename pattern â†’ AI confirmation |
