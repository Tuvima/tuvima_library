...
- Bridge identifiers: ISBN, ASIN, TMDB ID, Apple Books ID, MusicBrainz ID, Comic Vine ID, Apple Podcasts ID — these are used by Stage 2 to resolve the QID precisely
...
4. **Multiple candidates:** Multiple candidates without auto-accept → `MultipleQidMatches` review item. Conservative matching — no auto-accept when ambiguous.
...
- Full Data Extension deep hydration — all 50+ properties from `config/universe/wikidata.json`
...
1. User opens the Vault page in Settings → Metadata section.
