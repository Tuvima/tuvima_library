...
The `ReconciliationAdapter` runs second, using bridge IDs deposited by Stage 1 for precise QID resolution. The adapter is now a thin orchestrator over `Tuvima.Wikidata` v2.5.0 sub-services — the hand-rolled bridge resolution / music album resolution / text reconciliation helpers were deleted in the adapter slimdown remediation (see `.claude/plans/adapter-slimdown-remediation.md`):
...
3. **Text fallback (gated):** When a `BridgeStage2Request` returns NotFound and the input still has a Title + non-Unknown MediaType, the adapter issues a second-pass `TextStage2Request` with narrowed `CirrusSearchTypes` — `GetCirrusTypesForMediaType` prefers the narrow `work_classes` from the edition pivot config (e.g. Q7725634/Q571/Q8261 for Books) over the broad `instance_of_classes` list, preventing edition/series entities from polluting text search results. This mirrors the legacy `ResolveBridgeAsync`'s "if bridge IDs fail, try title+author" behaviour for parity. Sentinel-only requests (no real bridge IDs at all) return NotFound and route to review — **no retail = no Wikidata**.
...
7. **Post-resolution P31 validation:** After Data Extension claims are fetched, the adapter validates that the resolved entity's P31 (instance_of) is compatible with the requesting media type. `ValidateP31ForMediaType` checks two conditions: (a) the entity's P31 must not appear in `exclude_classes` for the media type (e.g. Q11424/film is excluded for Books), and (b) at least one P31 must appear in `instance_of_classes` for the media type. If validation fails, the result is discarded — the text fallback (item 3) retries with narrower CirrusSearch types. This prevents cross-type contamination where bridge IDs (especially ISBNs shared across adaptations) resolve to the wrong entity type (e.g. a book ISBN resolving to a film adaptation's QID). When no P31 data is returned, validation is permissive (passes). `P31` was added to `Stage2BridgePCodes` specifically for this validation.
8. **Wikipedia descriptions:** Fetched via `_reconciler.GetWikipediaSummariesAsync` as part of Stage 2.
