...
Universe and fictional-entity administration is rendered as content inside `SharedMediaEditorShell`, not as a separate route, dialog, or media workbench. Its launch request supplies a typed `SharedEntityEditorTargetDto` and does not need a media `EntityIds` list. The root presents the ordered entity categories from the Engine and bounded, searchable selectors; opening an entity retargets the same shell, and its breadcrumb returns to the Universe root. The shell composes sections from the target's readable capabilities, so graph targets never gain the physical-media Files section. Details and managed EntityAsset Artwork use the authorized shared-editor endpoints; Appearances, qualified Relationships, in-universe Timeline, Sources/provenance, History, and enrichment projections remain capability-driven and read-only. In-universe Timeline is narrative chronology, distinct from editor History. These controls do not alter authored-container canonical title/order or media cross-navigation paths.
