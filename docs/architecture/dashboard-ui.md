...
Series and collection containers do not use that expansion behavior. `MediaGroupTile` is a dedicated fixed-size landscape renderer whose resting state composes artwork from owned child items. Hover or keyboard focus replaces the resting state in place with a previous/selected/next carousel without changing card dimensions or shelf geometry. The resting surface opens the group; the selected artwork and primary carousel action open the selected child. TV show previews carry owned episode stills and show-scoped episode routes, while ordered book, comic, movie, audiobook, and album previews preserve their Engine order. Ordinary media and Continue cards remain on their existing renderers.
