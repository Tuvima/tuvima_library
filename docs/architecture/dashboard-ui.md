...
`LibraryBrowsePage` is the only landing page that composes `CinematicHeroCarousel`. `CinematicHeroSurface` remains the outer renderer for both the Home carousel and `DetailHero`, and both render identity/content through `DetailHeroContent`. `MediaLaneHeader` owns the persistent lane identity and `SurfaceNavigationBar` on Read, Watch, and Listen browse surfaces. `MediaHubPage` supplies the compact row-based Discover content beneath it, while `MediaBrowseShell` supplies the complete URL-backed genre, creator, year, progress, sorting, layout, and compact Continue controls beneath the same header on direct scope routes. Those scope tabs resolve real routes rather than filtering a subset of loaded shelves in memory. Collections is browse-first and does not render a hero.
...
Non-TV series and collection containers do not use that expansion behavior. `MediaGroupTile` is the single wide, fixed-size landscape renderer for both legacy and display-API group sources. Its resting state is artwork-only. Two, three, or four representative owned images form a slightly angled, overlapping cluster based on their real portrait, square, or wide metadata; images keep their natural ratios and only four are loaded and rendered. Hover or keyboard focus does not replace, reorder, or move the artwork and does not change card dimensions or shelf geometry. It adds the purple boundary/glow and fades in a lightly shaded compact top overlay containing only container type, a two-line-clamped title, and one status concept such as Continue, ownership, completion, or item/media-type count. The entire surface opens the full group. There are no buttons, arrows, dots, artwork rotation, selected-child state, or child-level actions. TV shows are an explicit exception: they render through `MediaTile` with the show cover at rest, then expand to the show-level cinematic backdrop, logo, facts, and description on hover. Owned episode navigation remains on the show detail surface. Ordinary media and Continue cards remain on their existing renderers.
