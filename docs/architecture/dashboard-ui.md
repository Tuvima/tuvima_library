...
Detail hero actions and media-tile hovers use the same action language and shared button geometry. The primary verb is contextual—Read for books/comics, Play for movies/TV, and Listen for music/audiobooks—while My List, Not for me, Like, Love, Add to Collection, and Details retain the same meaning, order, icon, selected state, and flat squarish styling across lanes. Detail pages use the full-size action variant; in-row hovers use its compact variant and reserve a fixed bottom action anchor so titles and artwork cannot move the controls. My List is the universal profile shortlist backed by the existing saved/favorites collection; Love remains a separate preference reaction. In-progress watch titles expose Resume and Restart together, and lane-specific tools such as Shuffle or Watch Party follow the universal actions. Backdrops render as image elements rather than repeated CSS backgrounds: an edge-to-edge cover layer fills the 1000px canvas while an uncropped contain layer preserves the complete enriched frame above it.
