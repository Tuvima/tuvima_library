...
Non-TV series and collection containers do not use that expansion behavior. `MediaGroupTile` is a dedicated wide, fixed-size landscape renderer. The left text column and right artwork region keep the same alignment at rest and on hover. Two, three, or four representative owned covers occupy one side-by-side row on the right; their equal-width slots become narrower as the count grows, while each complete cover scales proportionally and remains vertically centered without cropping or distortion. Only the first four are shown. Hover or keyboard focus adds a static container summary in place without changing card dimensions or shelf geometry, and the copy sits directly on the card atmosphere rather than in a nested shaded or bordered panel. The group type and name stay primary. `At a glance` rows report truthful media counts for collections or sequence ownership and progress for series; they never repeat the visible child titles. One action opens the full group. There are no arrows, dots, artwork rotation, selected-child state, or child-level actions. The shared shelf snap logic treats group tiles as first-class row items so arrows move between actual card boundaries instead of skipping to the row end. TV shows are an explicit exception: they render through `MediaTile` with the show cover at rest, then expand to the show-level cinematic backdrop, logo, facts, description, and actions on hover. Owned episode navigation remains on the show detail surface. Ordinary media and Continue cards remain on their existing renderers.
...
`MediaArtworkGroupPreview` is the shared non-interactive representative-art primitive. Its group-card strip renders two to four unique managed images in equal grid columns and suppresses overflow badges; complete covers scale proportionally and remain vertically centered rather than stretching, cropping, wrapping, or stacking as the count increases. Its clustered variant remains available for ordinary collection artwork contexts and can show `+N` from the real container total. The previous `MediaArtworkCarousel` and JavaScript-rotating collage were removed and must not be reintroduced on card or detail surfaces; deliberate item browsing belongs in the existing detail lists and selectors.
