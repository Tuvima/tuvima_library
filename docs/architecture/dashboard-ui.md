...
- Media, people, series, and standard collections share one canonical full-width detail surface. Its desktop `DetailStage` targets 70svh and caps at 760 px. Album tracks and audiobook chapters share the bounded stage with the cinematic identity and scroll internally above shared navigation. Structural arrays—owned episodes, ordered sequences, standard collection items, appearances, and a person's **Works in Your Library**—begin below the stage and remain natural-height so they cannot cut the hero or be clipped by it. Portrait covers remain restrained inside the larger cinematic field, while long sequences retain their named Jump to selector. Playlists remain specialized Listen queue/edit surfaces.
