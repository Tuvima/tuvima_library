...
Series and collection containers do not use that expansion behavior. `MediaGroupTile` is a dedicated fixed-size landscape renderer whose resting state composes artwork from owned child items. The resting preview uses count- and shape-aware arrangements for two, three, four, and larger groups so portrait covers, square album art, wide episode stills, and mixed collections use the available height. Hover or keyboard focus replaces the resting state in place with an identity/action column and a previous/selected/next carousel without changing card dimensions or shelf geometry. The resting surface opens the group; the selected artwork and child action open the selected owned item. TV show previews carry owned episode stills and show-scoped episode routes, while ordered book, comic, movie, audiobook, and album previews preserve their Engine order. Ordinary media and Continue cards remain on their existing renderers.
