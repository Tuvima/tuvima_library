...
- Media, people, series, and standard collections share one canonical full-width detail surface. Its desktop `DetailStage` targets 70svh and caps at 760 px; the existing cinematic identity occupies the upper region, an Engine-selected primary module occupies the remaining bounded space, and shared navigation sits in its own padded shelf at the bottom. Track and chapter rows scroll inside that bounded module with sticky controls. Playlists remain specialized Listen queue/edit surfaces.
- Primary modules are explicit detail-contract data rather than route-driven UI guesses. They present tracks, chapters, owned episodes, ordered sequences, standard collection items, appearances, or a person's **Works in Your Library**. Person works come only from canonical eligible credits for owned media, deduplicate the work while retaining every eligible role, and can be filtered by Read, Watch, Listen, or role.
