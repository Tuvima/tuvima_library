...
Ordered series cards use `DisplayCardDto.PreviewItems` and `PreviewTotalCount` to expose the first owned works in display order. Each preview carries its media type, semantic web route, description, and media-specific facts so a group card can open the selected child and update its hover copy without inferring parent/child data in the client. TV show cards may still carry owned episode previews and counts for show-detail context, but Dashboard card hover preserves the show identity and uses show-level cinematic artwork rather than rendering those previews as a collection carousel. Clients should render non-TV sequence previews in order; the active title plus `n of total` pagination is sufficient in interactive carousels, while broader curated collections can still use decorative resting stacks because they are not sequence previews.
