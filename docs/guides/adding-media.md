...
This guide walks you through getting media files into Tuvima Library — whether you're adding a single new file or bringing in an existing collection of thousands.
...
**Watch mode** monitors a folder on an ongoing basis. The moment a new file appears — whether you copied it, downloaded it, or moved it there — the Engine picks it up and processes it automatically. Use this for your active download or import folders.
...
4. Fill in the folder details:
   - **Source path** — the folder on your drive to watch or import (e.g. `D:\Downloads\Books`).
   - **Library root** — where the organised library should live after files are processed (e.g. `D:\Library`). This is where Tuvima moves files once they've been identified.
   - **Category** — the type of media in this folder (Books, Movies, TV, Music, Comics, Podcasts).
   - **Media types** — the specific formats to accept (e.g. EPUB and PDF for a Books folder). You can select multiple.
   - **Watch mode** — toggle this on if you want the folder monitored continuously.
5. Click **Save**.

You can add as many folders as you need — one for each category, or one per drive, or however your collection is organised.
...
> **Note on MP3 and MP4:** These formats can belong to more than one library type — an MP3 might be an audiobook chapter or a music track; an MP4 might be a movie or a TV episode. The Engine uses AI classification and file metadata signals to decide. If it gets it wrong, you can correct it in the Library Vault.

---

## What happens when a file arrives

When the Engine finds a new file, it runs it through several stages automatically. You don't need to do anything — this all happens in the background.

1. **Settle** — The Engine waits a moment to make sure the file has finished copying before touching it. This prevents half-written files from being processed.

2. **Fingerprint** — A unique identifier is computed from the file's contents (a SHA-256 hash). This acts like a barcode — the Engine can track the file even if you rename or move it later.

3. **Scan** — The file is opened and any embedded information is read out: title, author, year, cover art, series name, narrator, and so on. Different processors handle different formats (EPUBs, ID3 tags in MP3s, MKV metadata, and so on).

4. **Identify** — The Engine scores the metadata it found and tries to match the file to an existing entry in your data store or to a known external source.

5. **Stage** — The file is held temporarily while enrichment runs.

6. **Hydrate** — Two rounds of enrichment fill in missing or uncertain metadata. Stage 1 queries retail providers (Apple, Google Books, TMDB, and others) to gather cover art, ratings, and identifiers. Stage 2 uses Wikidata to confirm the canonical identity and fetch structured data: author, director, series, genre, and more.

7. **Promote** — If the Engine is sufficiently confident (85% or above), the file is moved from staging into your organised library folder. If confidence falls below that threshold, the item is flagged for your review in the Library Vault.
...
1. Add the folder in **Settings → Library → Library Folders** as described above.
...
**Duplicates are handled gracefully.** If you add a file that the Engine has seen before (same fingerprint), it is recognised as a duplicate. If you add a different edition of a Work already in your library — say, a new audiobook version of a book you already have — the Engine creates a new Edition under the existing Work rather than treating it as a completely separate item.
